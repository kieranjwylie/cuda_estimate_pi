...
int main(int argc, char *argv[]) {    
    ...
    CHECK_IERR(read_command_line(argc, argv, &threads, &blocks, &points_per_thread, &output_max_threads));
    ...
    CHECK_IERR(estimate_pi(points_per_thread, threads, blocks, 1234UL, &milliseconds, &pi, &total_points));

    // Output results in format:
    // total threads, total points, esimated pi, error, time taken
    double error = fabs(pi - PI);
    std::cout << std::setprecision(15) << std::fixed;
    std::cout << threads*blocks << ", " << total_points << ", " << pi << ", " << error << ", " << milliseconds << std::endl;
